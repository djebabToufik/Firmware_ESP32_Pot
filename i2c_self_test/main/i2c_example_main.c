...
static esp_err_t i2c_master_write_slave(i2c_port_t i2c_num, uint8_t *data_wr, size_t size)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    rett=i2c_master_write_byte(cmd, (ESP_SLAVE_ADDR << 1) | WRITE_BIT, ACK_CHECK_EN);
   // printf("write slave ret = %s\n",esp_err_to_name(rett));
...
static esp_err_t i2c_master_sensor_test(i2c_port_t i2c_num, uint8_t *data_h, uint8_t *data_l)
{
    ...
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(i2c_num, cmd, 3000 / portTICK_RATE_MS);
    i2c_cmd_link_delete(cmd);
    return ret;
}
...
{
    uint8_t Tempaddr=0xF3;
    uint8_t read={0x00,0x00};

    int ret;
    while(1)
    {
        ret= i2c_master_write_slave(I2C_MASTER_NUM,&Tempaddr,1);
         if(ret!=0)
         {
    ...
    ret= i2c_master_read_slave(I2C_MASTER_NUM,&read,2);
  // printf("ESP read %s \n",esp_err_to_name(ret));
   

if(ret!=0)
   {
       printf("ESP_NOT_OK:  RHread %s \n",esp_err_to_name(ret));
   }
   else
   {
       printf("Value RH is %02x \n",read);
   }
